...
function buildReceiptHTML(r, forDownload = false) {
  ...
  const amountPaid = parseFloat(r.amount_paid || r.grand_total);

  const discountRows = discount > 0 ? `
    <div class="total-row" style="padding:6px 0;border-top:none;">
      <span style="font-size:13px;color:#111;">Subtotal</span>
      <span style="font-size:13px;">₦${fmt(r.grand_total)}</span>
    </div>
    <div class="total-row" style="padding:6px 0;border-top:none;">
      <span style="font-size:13px;color:#16a34a;">Discount</span>
      <span style="font-size:13px;color:#16a34a;">-₦${fmt(discount)}</span>
    </div>
    <div class="total-row">
      <span class="total-label">AMOUNT PAID</span>
      <span class="total-amount">₦${fmt(amountPaid)}</span>
    </div>` : `
    <div class="total-row">
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .receipt-meta { font-size: 12px; margin-bottom: 4px; }
